...
class ReferrerController extends Controller
{
    public function index(Request $request): JsonResponse
    {
        ...
        if ($request->filled('active')) $query->where('active', filter_var($request->input('active'), FILTER_VALIDATE_BOOLEAN));
    ...
    public function exportPdf(ExportPDF $pdfService)
    {
        ...
        if ($rows->isEmpty()) return response()->json(['message' => 'No referrers found.'], 404);
        $title = 'Referrers';
        $headers = [
            'id' => 'ID',
            'name' => 'Name',
